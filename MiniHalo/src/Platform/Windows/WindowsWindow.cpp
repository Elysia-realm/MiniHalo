...
namespace MiniHalo
{
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		HA_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			int success = glfwInit();
			HA_COER_ASSERT(success, "Could not initialize GLFW!");
		...
		HA_COER_ASSERT(status, "Failed to initialize Glad!");
